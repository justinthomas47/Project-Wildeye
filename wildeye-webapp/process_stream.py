...
duplicate_stats = {"count": 0, "last_log_time": 0, "by_camera": {}}
...
def download_youtube_video(url, seek_time=0, max_duration=None):
            ...
            try:
                ...
                if cap.isOpened():
                    ...
                    if ret:
                        ...
                        logger.warning(
                            f"Cached video exists but cannot be read, re-downloading"
                        )
                        os.remove(video_path)
                else:
                    logger.warning(
                        f"Cached video exists but cannot be opened, re-downloading"
                    )
                    os.remove(video_path)
            except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if os.path.exists(temp_video_path):
                ...
                if cap.isOpened():
                    ...
                    if ret:
                        ...
                        logger.info(
                            f"Successfully downloaded high quality video to {video_path}"
                        )
                    else:
                        logger.warning(
                            f"Downloaded file cannot be read, will try lower quality"
                        )
                else:
                    logger.warning(
                        f"Downloaded file cannot be opened, will try lower quality"
                    )
            else:
            ...
            logger.warning(
                f"Error during high quality download: {e}, trying lower quality"
            )
        ...
        if not download_success:
            try:
                ...
                if os.path.exists(temp_video_path):
                    ...
                    download_success = True
                    logger.info(
                        f"Successfully downloaded lower quality video to {video_path}"
                    )
            except Exception as e:
...
def clean_youtube_cache(max_size_gb=5):
    ...
    try:
        ...
        if total_size_gb > max_size_gb:
            logger.info(
                f"YouTube cache size ({total_size_gb:.2f} GB) exceeds limit ({max_size_gb} GB). Cleaning up..."
            )
...
def is_recent_duplicate(
    camera_id: str, detection_label: str, current_time: datetime
) -> bool:
    """
            ...
            if time_difference < timedelta(minutes=5):
                ...
                if current_time_seconds - duplicate_stats["last_log_time"] > 30:
                    ...
                    for cam_id, animals in duplicate_stats["by_camera"].items():
                        animal_counts = ", ".join(
                            [f"{animal}: {count}" for animal, count in animals.items()]
                        )
                        logger.info(
                            f"Skipped duplicates for camera {cam_id}: {animal_counts}"
                        )
...
def add_detection(db, detection_data: Dict, screenshot: bytes = None) -> Optional[str]:
    ...
    try:
        ...
        if is_recent_duplicate(
            detection_data["camera_id"], detection_data["detection_label"], current_time
        ):
            return None
        ...
        try:
            ...
            if warning_id:
                logger.info(
                    f"Created warning {warning_id} for detection {detection_id}"
                )
        ...
        logger.info(
            f"Added new detection: {detection_data['detection_label']} from {camera['camera_name']}"
        )
        return detection_id
    except Exception as e:
...
def get_detections_by_camera(
    db, camera_id: str, limit: int = 100, user_id: str = None
) -> List[Dict]:
    """
...
class VideoProcessor:
    def __init__(self, model_path: str, num_workers: int = 4, queue_size: int = 30):
        ...
        self.input_type = None  # Store input type (youtube_link, file_upload, etc.)
        self.need_first_frame = False  # Flag to read first frame even when paused
        self.video_looped = False  # Track if we've looped a video file
    ...
    def _frame_prefetcher(self, cap: cv2.VideoCapture) -> None:
        ...
        is_file = self.input_type == "file_upload"
        
        logger.info(f"Starting frame prefetcher thread (YouTube: {is_youtube}, File: {is_file}, FPS: {video_fps})")
        
        while not self.stop_flag.is_set():
            try:
                current_buffer_size = self.prefetch_buffer.qsize()
                
                # Control buffer filling rate based on input type
                if is_file:
                    # For file uploads, slower reading avoids overwhelming system
                    if current_buffer_size >= buffer_target * 0.5:  # Fill only up to 50% for files
                        time.sleep(0.05)
                        continue
                    
                    # Cap frames_needed lower for files to avoid reading too fast
                    frames_needed = min(5, buffer_target - current_buffer_size)
                else:
                    # For streams like YouTube, we want a fuller buffer
                    if current_buffer_size >= buffer_target * 0.8:
                        time.sleep(0.1)
                        continue
                ...
                for _ in range(frames_needed):
                    ...
                    if not ret:
                        ...
                        # For file uploads, loop back to beginning
                        if is_file and consecutive_failures >= 3 and consecutive_failures < max_failures:
                            try:
                                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                                logger.info("Prefetcher: Looped video back to beginning")
                                consecutive_failures = 0
                                continue
                            except Exception as e:
                                logger.error(f"Prefetcher: Error looping video: {e}")
                ...
                # Control timing based on video frame rate and input type
                elapsed = time.time() - last_frame_time
                if is_file:
                    # For file uploads, we need to be more careful with timing
                    if elapsed < target_frame_time * 2 and frames_needed == 0:
                        time.sleep(min(0.1, target_frame_time * 2 - elapsed))
                else:
                    # For streams, normal timing
                    if elapsed < target_frame_time and frames_needed == 0:
                        time.sleep(min(0.1, target_frame_time - elapsed))
    ...
    def _frame_producer(self, cap: cv2.VideoCapture) -> None:
        ...
        elif self.input_type == "file_upload":
            # For file uploads, we might need to skip frames if system is slow
            frame_skip = 2  # Default to every 2nd frame
            logger.info(f"File upload detected, processing every {frame_skip}th frame")
        ...
        while not self.stop_flag.is_set():
            try:
                # Special case: if we need the first frame for a paused file upload
                if self.need_first_frame:
                    ret, frame = cap.read()
                    if ret:
                        ...
                        try:
                            ...
                            logger.error(f"Queue error (first frame): {queue_error}")
                    self.need_first_frame = False
                    continue
                ...
                if not ret:
                    ...
                    # Handle end-of-file looping for uploaded videos
                    if self.input_type == "file_upload":
                        # Reset video to beginning if we've reached the end
                        if consecutive_failures >= 3 and consecutive_failures < max_failures:
                            try:
                                # Reset to beginning
                                cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                                self.video_looped = True
                                logger.info("Looped video back to beginning")
                                consecutive_failures = 0
    ...
    def _process_frame(self, worker_id: int, frame_data: tuple) -> tuple:
        ...
        try:
            ...
            # Also don't log detections when the video has looped (to avoid duplicates)
            if detections and self.camera_id and self.db and not self.pause_flag.is_set() and not self.video_looped:
                try:
                    # Convert the processed frame to JPEG format - use lower quality for better performance
                    encode_params = [cv2.IMWRITE_JPEG_QUALITY, 80]  # 80% quality is good enough
    ...
    def process_stream(self, input_type: str, input_value: str, seek_time: int = 0, camera_id: str = None, db = None, is_visible: bool = False) -> Generator:
        ...
        self.video_looped = False  # Reset looping flag
        
        # Set initial pause state based on visibility, but always read at least one frame for file uploads
        if not is_visible:
            logger.info(f"Starting stream in paused state (not visible): {input_value}")
            self.pause_flag.set()
            # For file uploads, we'll still need to read the first frame
            self.need_first_frame = input_type == "file_upload"
        else:
        ...
        try:
            ...
            if input_type == "file_upload":
                # For file upload, input_value should be the file path
                stream_source = input_value
                logger.info(f"Opening uploaded video file: {stream_source}")
            ...
            if input_type == "youtube_link":
                ...
                # For file uploads, also use a larger buffer
                cap.set(cv2.CAP_PROP_BUFFERSIZE, 10)  # Even larger for files
            ...
            # Cap FPS to a reasonable value for smooth playback
            target_fps = min(30, self.fps)  # Cap at 30fps for better performance
            logger.info(f"Video FPS: {self.fps}, Target FPS: {target_fps}")
            ...
            if seek_time > 0:
                if input_type == "youtube_link":
                    ...
                    # For uploaded files, seeking should work well
                    frame_number = int(seek_time * self.fps)
                    cap.set(cv2.CAP_PROP_POS_FRAMES, frame_number)
                    logger.info(f"Seeked to frame {frame_number} (time: {seek_time}s)")
            ...
            # Use fewer workers for file uploads to reduce overhead
            if input_type == "file_upload":
                worker_count = 2  # Use fewer workers for file uploads
            else:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=worker_count) as executor:
                ...
                adaptive_target_time = target_frame_time
                frame_times = []  # Keep track of recent frame times

                while not self.stop_flag.is_set():
                    try:
                        processed_frame, detections, timestamp = self.result_queue.get(timeout=0.5)

                        # Strict frame rate control for smooth playback
                        current_time = time.time()
                        elapsed = current_time - last_frame_time
                        sleep_time = adaptive_target_time - elapsed
                        ...
                        actual_fps = 1.0 / max(0.001, actual_frame_time)  # Avoid division by zero
                        
                        # Keep track of recent frame times for adaptive timing
                        frame_times.append(actual_frame_time)
                        if len(frame_times) > 10:
                            frame_times.pop(0)
                        
                        # Calculate average frame time over last 10 frames
                        avg_frame_time = sum(frame_times) / len(frame_times) if frame_times else actual_frame_time
                        
                        # Adaptive timing adjustment - if we're consistently too slow or too fast, adjust target
                        if frame_count > 30 and len(frame_times) >= 5:
                            if avg_frame_time > adaptive_target_time * 1.2:
                                # Too slow, increase target time (slow down target FPS)
                                adaptive_target_time = min(0.1, adaptive_target_time * 1.05)  # Max 10 FPS min
                                logger.debug(f"Slowing target FPS to {1.0/adaptive_target_time:.2f}")
                            elif avg_frame_time < adaptive_target_time * 0.8 and adaptive_target_time > target_frame_time:
                                # Too fast but only if we previously slowed down
                                adaptive_target_time = max(target_frame_time, adaptive_target_time * 0.95)
                                logger.debug(f"Speeding up target FPS to {1.0/adaptive_target_time:.2f}")

                        if processed_frame is not None:
                            # Encode frame as JPEG with quality control
                            encode_params = [cv2.IMWRITE_JPEG_QUALITY, 80]  # Lower quality for better performance
                            _, buffer = cv2.imencode('.jpg', processed_frame, encode_params)
                            ...
                            # Log frame rate less frequently to reduce overhead
                            if frame_count % 100 == 0:
                                logger.info(f"Stream delivery rate: {actual_fps:.2f} fps (target: {1.0/adaptive_target_time:.2f})")
                        ...
                        if hasattr(self, 'last_processed_frame') and self.last_processed_frame is not None:
                            try:
                                ...
                                _, buffer = cv2.imencode('.jpg', self.last_processed_frame, encode_params)
                                frame = buffer.tobytes()
                                yield (b'--frame\r\n'
                                    b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')
                            except Exception as e:
...
def process_stream(
    ...
    logger.info(
        f"Processing stream request - Type: {input_type}, Value: {input_value}, Visible: {is_visible}"
    )
    try:
        yield from video_processor.process_stream(
            input_type, input_value, seek_time, camera_id, db, is_visible
        )
    except Exception as e:
...
def pause_stream(camera_id: str):
    ...
    if video_processor is not None:
        logger.info(f"Pausing stream for camera ID: {camera_id}")
...
def resume_stream(camera_id: str):
    ...
    global video_processor
    if video_processor is not None:
        logger.info(f"Resuming stream for camera ID: {camera_id}")
